...
const PaceManagement = () => {
  ...
  useEffect(() => {
    ...
    if (isRecording && !isPaused) {
      ...
      // Real-time voice analysis every 3 seconds
      voiceAnalysisTimer = setInterval(() => {
        analyzeRealtimeVoice();
      }, 3000);
    }
  ...
  const analyzeRealtimeVoice = async () => {
    if (!isRecording || isPaused || !mediaStreamRef.current || isAnalyzingVoice) return;

    setIsAnalyzingVoice(true);
    try {
      // Use the existing audio context from startAnalyzing if available
      if (!audioContextRef.current || audioContextRef.current.state === 'closed') {
        setIsAnalyzingVoice(false);
        return; // Skip analysis if no valid audio context
      }

      // Create a temporary audio context for recording 3 seconds of audio
      ...
      // Record for 2 seconds to reduce blob size
      setTimeout(async () => {
        if (!isRecording || isPaused) {
          ...
          try {
            ...
            console.warn("Error cleaning up voice analysis:", e);
          }
      ...
      console.warn("Real-time voice analysis error:", error);
      // Reset to 0 on error
      setRealtimeVoiceMetrics({
        jitter_local: 0,
        shimmer_local: 0,
  ...
  // Helper function to create WAV blob (from Loudness.jsx)
  ...
  const sendVoiceQualityAnalysis = async (wavBlob) => {
    try {
      ...
      if (result.advancedMetrics) {
        // Extract voice quality metrics from advancedMetrics section
        ...
        // Use the actual backend values directly for speech_continuity and speaking_efficiency
        const actualSpeechContinuity = metrics.speech_continuity || 0;
        const actualSpeakingEfficiency = metrics.speaking_efficiency || 0;
        
        // If both are very high (>0.95), it might mean no speech detected, so check voice quality
        ...
        if (jitter === 0 && shimmer === 0 && hnr === 0) {
          ...
          // Very high values might indicate no pauses, but we should still consider voice quality
        ...
        setRealtimeVoiceMetrics({
          ...
          hnr_mean: hnr,
          f1_mean: metrics.f1_mean || 0,
          rhythm_outliers: rhythmOutliers,
    ...
    return () => {
      ...
      if (audioContextRef.current) {
        try {
          ...
          // Ignore close errors - AudioContext might already be closed
          console.debug("AudioContext cleanup error:", closeError.message);
        }
      }
    };
  }, []);
  ...
  useEffect(() => {
    if (isRecording && !isPaused) {
      ...
      // Start voice quality analysis every 5.5 seconds
      const voiceQualityInterval = setInterval(() => {
        if (isRecording && !isPaused) {
          analyzeRealtimeVoice();
        }
      }, 5500);
      ...
      // Reset voice metrics when not recording or paused
      setRealtimeVoiceMetrics({
        jitter_local: 0,
        shimmer_local: 0,
        hnr_mean: 0,
  ...
  const handlePause = () => {
    if (mediaRecorderRef.current && isRecording && !isPaused) {
      ...
      // Reset real-time metrics when pausing
      setRealtimeVoiceMetrics({
        jitter_local: 0,
        shimmer_local: 0,
        hnr_mean: 0,
  ...
  const handleStop = () => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      // Reset real-time metrics to 0 when stopping
      setRealtimeVoiceMetrics({
        jitter_local: 0,
        shimmer_local: 0,
        hnr_mean: 0,
  ...
  const analyzeAudio = async (blob, setResults) => {
    ...
    try {
      ...
      if (pauseData.error) {
        ...
        // Use backend prediction only if it's valid, otherwise use frontend calculation
        const finalLabel = (backendLabel && isValidBackendPrediction(backendLabel, rateData.wpm)) 
          ? backendLabel 
          : frontendCalculatedLabel;
        

        // Log warning if backend prediction was invalid
        if (backendLabel && !isValidBackendPrediction(backendLabel, rateData.wpm)) {
          // console.warn(`⚠️ Backend model prediction "${backendLabel}" doesn't match WPM ${rateData.wpm}, using frontend calculation instead`);
        }
        ...
        // Calculate frontend consistency score if backend is invalid
        let finalConsistencyScore = rateData.consistencyScore || 0;
        if (finalConsistencyScore <= 0 && rateData.pacingCurve && rateData.pacingCurve.length >= 3) {
          // Calculate consistency from pacing curve
          const wpmValues = rateData.pacingCurve.map(point => point.wpm || 0);
          const wpmStd = Math.sqrt(wpmValues.reduce((acc, wpm, i, arr) => {
            const mean = arr.reduce((a, b) => a + b, 0) / arr.length;
            return acc + Math.pow(wpm - mean, 2);
          }, 0) / wpmValues.length);
          finalConsistencyScore = Math.max(0, 100 - wpmStd);
          // console.log(`Calculated frontend consistency score: ${finalConsistencyScore}% (from WPM std: ${wpmStd})`);
        }
        ...
        if (backendRateFeedback) {
          ...
          frontendRateFeedback = generateFrontendFeedback(
            ...
            finalConsistencyScore, // Use calculated consistency score
        ...
        setResults({
          ...
          consistencyScore: finalConsistencyScore, // Use corrected consistency score
          pacingCurve: rateData.pacingCurve || [],
          ...
          backendPredictionValid: isValidBackendPrediction(backendLabel, rateData.wpm), // Track if backend prediction was valid
  ...
  const getWpmLabel = (wpm) => {
    ...
    if (!wpm || wpm <= 0 || wpm > 500) {
      // console.warn(`⚠️ Invalid WPM value: ${wpm}, defaulting to "Slow"`);
      return "Slow";
    }
  ...
  // Calculate label probability based on feature values and thresholds from auto_label function
  const calculateLabelProbability = (label, metrics) => {
    if (!metrics) return 0;

    // Extract feature values with defaults
    const pauseRatio = metrics.pause_ratio || 0;
    const excessiveCount = metrics.excessive_count || 0;
    const contextualPauseScore = metrics.contextual_pause_score || 0;
    const wpmCv = metrics.wpm_cv || 0;
    const wpmDeltaStd = metrics.wpm_delta_std || 0;
    const segDurationStd = metrics.seg_duration_std || 0;
    const wpmJerk = metrics.wpm_jerk || 0;
    const rhythmOutliers = metrics.rhythm_outliers || 0;
    const pauseRhythmConsistency = metrics.pause_rhythm_consistency || 1.0;
    const speechContinuity = metrics.speech_continuity || 1.0;
    const goldenRatioPauses = metrics.golden_ratio_pauses || 0;
    const cognitivePauseScore = metrics.cognitive_pause_score || 0;
    const confidenceScore = metrics.confidence_score || 1.0;
    const optimalCognitivePauseRatio = metrics.optimal_cognitive_pause_ratio || 1.0;
    const speakingEfficiency = metrics.speaking_efficiency || 1.0;
    const pauseEfficiency = metrics.pause_efficiency || 0;
    const pauseSpacingConsistency = metrics.pause_spacing_consistency || 1.0;
    const toastmastersScore = metrics.toastmasters_score || 0;
    const optimalTransitionRatio = metrics.optimal_transition_ratio || 0;

    // Calculate probability based on how close the current values are to the thresholds
    switch (label) {
      case "poor_pause_control":
        // pause_ratio > 0.15 OR excessive_count > 2 OR contextual_pause_score < -0.5
        const poorPauseScore = Math.max(
          pauseRatio > 0.15 ? 1.0 : pauseRatio / 0.15,
          excessiveCount > 2 ? 1.0 : excessiveCount / 2,
          contextualPauseScore < -0.5 ? 1.0 : Math.max(0, (0.5 + contextualPauseScore) / 0.5)
        );
        return Math.min(poorPauseScore, 1.0);

      case "needs_pause_improvement":
        // pause_ratio > 0.12 OR excessive_count > 1 OR contextual_pause_score < 0.0
        const needsImprovementScore = Math.max(
          pauseRatio > 0.12 ? 1.0 : pauseRatio / 0.12,
          excessiveCount > 1 ? 1.0 : excessiveCount / 1,
          contextualPauseScore < 0.0 ? 1.0 : Math.max(0, (0.2 + contextualPauseScore) / 0.2)
        );
        return Math.min(needsImprovementScore, 1.0);

      case "inconsistent_pace":
        // wpm_cv > 0.4 OR wpm_delta_std > 8 OR seg_duration_std > 2.5 OR wpm_jerk > 10
        const inconsistentPaceScore = Math.max(
          wpmCv > 0.4 ? 1.0 : wpmCv / 0.4,
          wpmDeltaStd > 8 ? 1.0 : wpmDeltaStd / 8,
          segDurationStd > 2.5 ? 1.0 : segDurationStd / 2.5,
          wpmJerk > 10 ? 1.0 : wpmJerk / 10
        );
        return Math.min(inconsistentPaceScore, 1.0);

      case "moderate_pace_consistency":
        // wpm_cv > 0.25 OR wpm_delta_std > 5 OR wpm_jerk > 5
        const moderatePaceScore = Math.max(
          wpmCv > 0.25 ? 1.0 : wpmCv / 0.25,
          wpmDeltaStd > 5 ? 1.0 : wpmDeltaStd / 5,
          wpmJerk > 5 ? 1.0 : wpmJerk / 5
        );
        return Math.min(moderatePaceScore, 1.0);

      case "poor_rhythm":
        // rhythm_outliers >= 6 OR pause_rhythm_consistency < 0.3 OR speech_continuity < 0.6 OR golden_ratio_pauses < 0.2
        const poorRhythmScore = Math.max(
          rhythmOutliers >= 6 ? 1.0 : rhythmOutliers / 6,
          pauseRhythmConsistency < 0.3 ? 1.0 : Math.max(0, (0.3 - pauseRhythmConsistency) / 0.3),
          speechContinuity < 0.6 ? 1.0 : Math.max(0, (0.6 - speechContinuity) / 0.6),
          goldenRatioPauses < 0.2 ? 1.0 : Math.max(0, (0.2 - goldenRatioPauses) / 0.2)
        );
        return Math.min(poorRhythmScore, 1.0);

      case "moderate_rhythm":
        // rhythm_outliers >= 3 OR pause_rhythm_consistency < 0.5 OR speech_continuity < 0.8 OR golden_ratio_pauses < 0.4
        const moderateRhythmScore = Math.max(
          rhythmOutliers >= 3 ? 1.0 : rhythmOutliers / 3,
          pauseRhythmConsistency < 0.5 ? 1.0 : Math.max(0, (0.5 - pauseRhythmConsistency) / 0.5),
          speechContinuity < 0.8 ? 1.0 : Math.max(0, (0.8 - speechContinuity) / 0.8),
          goldenRatioPauses < 0.4 ? 1.0 : Math.max(0, (0.4 - goldenRatioPauses) / 0.4)
        );
        return Math.min(moderateRhythmScore, 1.0);

      case "cognitive_load_issues":
        // cognitive_pause_score < 0.3 OR confidence_score < 0.6 OR optimal_cognitive_pause_ratio < 0.5
        const cognitiveLoadScore = Math.max(
          cognitivePauseScore < 0.3 ? 1.0 : Math.max(0, (0.3 - cognitivePauseScore) / 0.3),
          confidenceScore < 0.6 ? 1.0 : Math.max(0, (0.6 - confidenceScore) / 0.6),
          optimalCognitivePauseRatio < 0.5 ? 1.0 : Math.max(0, (0.5 - optimalCognitivePauseRatio) / 0.5)
        );
        return Math.min(cognitiveLoadScore, 1.0);

      case "needs_overall_improvement":
        // speaking_efficiency < 0.7 OR pause_efficiency > 3.0 OR pause_spacing_consistency < 0.5
        const overallImprovementScore = Math.max(
          speakingEfficiency < 0.7 ? 1.0 : Math.max(0, (0.7 - speakingEfficiency) / 0.7),
          pauseEfficiency > 3.0 ? 1.0 : pauseEfficiency / 3.0,
          pauseSpacingConsistency < 0.5 ? 1.0 : Math.max(0, (0.5 - pauseSpacingConsistency) / 0.5)
        );
        return Math.min(overallImprovementScore, 1.0);

      case "toastmasters_excellent":
        // toastmasters_score >= 0.8 AND pause_ratio >= 0.08 AND pause_ratio <= 0.12 AND optimal_transition_ratio >= 0.7
        const toastmastersExcellenceScore = (
          (toastmastersScore >= 0.8 ? 1.0 : toastmastersScore / 0.8) *
          (pauseRatio >= 0.08 && pauseRatio <= 0.12 ? 1.0 : 0.5) *
          (optimalTransitionRatio >= 0.7 ? 1.0 : optimalTransitionRatio / 0.7)
        );
        return Math.min(toastmastersExcellenceScore, 1.0);

      case "good_pace_control":
        // Default case - inverse of other problems (avoiding recursive calls)
        const otherProblems = [
          // poor_pause_control
          Math.max(
            pauseRatio > 0.15 ? 1.0 : pauseRatio / 0.15,
        ...
        const maxProblem = Math.max(...otherProblems);
        return Math.max(0, 1.0 - maxProblem);
          ...
          <div className="w-full flex flex-col">
            ...
            {activeTab === "advanced" && (
              <div className="flex flex-col w-full h-full">
                ...
                {/* Pause Model Labels Analysis */}
                <div className="mb-6 p-4 bg-gradient-to-r from-[#00171f] to-[#003b46] rounded-lg border-2 border-[#00ccff]/40">
                  <h3 className="text-[#00ccff] text-lg font-semibold mb-3">🏷️ Pause Model Labels Analysis</h3>
                  
                
                  
                  {/* Show only if we have prediction data */}
                  {results.pauseAnalysis.prediction ? (
                    <div className="space-y-4">
                    
                      {/* All Labels with their probabilities */}
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {[
                          { label: "poor_pause_control", description: "Poor Pause Control", color: "from-red-500 to-red-600" },
                          { label: "needs_pause_improvement", description: "Needs Pause Improvement", color: "from-orange-500 to-orange-600" },
                          { label: "inconsistent_pace", description: "Inconsistent Pace", color: "from-yellow-500 to-yellow-600" },
                          { label: "moderate_pace_consistency", description: "Moderate Pace Consistency", color: "from-blue-500 to-blue-600" },
                          { label: "poor_rhythm", description: "Poor Rhythm", color: "from-red-500 to-red-600" },
                          { label: "moderate_rhythm", description: "Moderate Rhythm", color: "from-orange-500 to-orange-600" },
                          { label: "cognitive_load_issues", description: "Cognitive Load Issues", color: "from-purple-500 to-purple-600" },
                          { label: "needs_overall_improvement", description: "Needs Overall Improvement", color: "from-yellow-500 to-yellow-600" },
                          { label: "toastmasters_excellent", description: "Toastmasters Excellent", color: "from-green-500 to-green-600" },
                          { label: "good_pace_control", description: "Good Pace Control", color: "from-green-500 to-green-600" }
                        ].map(({ label, description, color }) => {
                          // Get probability for this label from the model results (backend probabilities are authoritative)
                          let probability = results.pauseAnalysis.probabilities?.[label] || 0;
                          const isPredicted = results.pauseAnalysis.prediction === label;
                          
                          // Only use frontend calculation if backend probabilities are not available
                          if (!results.pauseAnalysis.probabilities || Object.keys(results.pauseAnalysis.probabilities).length === 0) {
                            probability = calculateLabelProbability(label, results.advancedMetrics);
                          }
                          
                          return (
                            <div key={label} className={`bg-white/10 rounded-lg p-4 ${isPredicted ? 'ring-2 ring-[#00ccff] bg-[#00ccff]/10' : ''}`}>
                              <div className="flex justify-between items-center mb-3">
                                <span className="text-white text-sm font-medium">
                                  {description}
                                </span>
                                {isPredicted && (
                                  <span className="text-[#00ccff] text-xs font-bold px-2 py-1 bg-[#00ccff]/20 rounded-full">
                                    PREDICTED
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="w-full bg-white/20 rounded-full h-4 relative overflow-hidden">
                                ...
                                {/* Probability text inside the bar */}
                    ...
                    <div className="text-center py-8">
                      <div className="text-white/70 text-lg mb-2">No Analysis Available</div>
                      <div className="text-white/50 text-sm">Record audio to see pause model predictions</div>
                    </div>
                  )}
                </div>
        ...
        {false && results.comprehensiveReport && typeof results.comprehensiveReport !== 'string' && (
          <div className="mt-6 p-4 lg:p-6 bg-gradient-to-b from-[#00171f] to-[#003b46] dark:from-[#003b46] dark:to-[#0084a6] rounded-lg">
            ...
            {activeTab === "voice" && (
              <div className="flex flex-col w-full h-full">
                <h2 className="text-xl lg:text-2xl font-bold text-white mt-2 mb-4">
                  Voice Quality & Rhythm Analysis
                </h2>

                {/* Real-time Voice Quality Detection */}
                <div className="mb-6 p-4 bg-gradient-to-r from-[#00171f] to-[#003b46] rounded-lg border-2 border-[#00ccff]/40">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-[#00ccff] text-lg font-semibold">🎤 Real-time Voice Analysis</h3>
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${isRecording ? 'bg-green-400 animate-pulse' : 'bg-gray-400'}`}></div>
                      <span className="text-white text-sm">
                        {isRecording ? 'Live Analysis' : 'Not Recording'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Voice Stability</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0).toFixed(3)}
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${
                            (isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0) < 0.02 
                              ? 'bg-gradient-to-r from-green-400 to-green-600' 
                              : (isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0) < 0.05
                              ? 'bg-gradient-to-r from-yellow-400 to-yellow-600'
                              : 'bg-gradient-to-r from-red-400 to-red-600'
                          }`}
                          style={{ 
                            width: `${Math.min(Math.max(((isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0) / 0.1) * 100, 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Voice Clarity</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0).toFixed(3)}
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${
                            (isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0) < 0.1 
                              ? 'bg-gradient-to-r from-green-400 to-green-600' 
                              : (isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0) < 0.2
                              ? 'bg-gradient-to-r from-yellow-400 to-yellow-600'
                              : 'bg-gradient-to-r from-red-400 to-red-600'
                          }`}
                          style={{ 
                            width: `${Math.min(Math.max(((isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0) / 0.3) * 100, 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                      <div className="text-white/60 text-xs">
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Voice Quality</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? realtimeVoiceMetrics.hnr_mean : 0).toFixed(1)} dB
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${
                            (isRecording && !isPaused ? realtimeVoiceMetrics.hnr_mean : 0) > 20 
                              ? 'bg-gradient-to-r from-green-400 to-green-600' 
                              : (isRecording && !isPaused ? realtimeVoiceMetrics.hnr_mean : 0) > 10
                              ? 'bg-gradient-to-r from-yellow-400 to-yellow-600'
                              : 'bg-gradient-to-r from-red-400 to-red-600'
                          }`}
                          style={{ 
                            width: `${Math.min(Math.max(((isRecording && !isPaused ? realtimeVoiceMetrics.hnr_mean : 0) / 30) * 100, 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                      <div className="text-white/60 text-xs">
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Formant F1</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? realtimeVoiceMetrics.f1_mean : 0).toFixed(0)} Hz
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          ...
                          style={{ 
                            width: `${Math.min(Math.max(((isRecording && !isPaused ? realtimeVoiceMetrics.f1_mean : 0) / 1000) * 100, 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                      <div className="text-white/60 text-xs">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Rhythm Consistency</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? (realtimeVoiceMetrics.rhythm_consistency * 100) : 0).toFixed(1)}%
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${
                            (isRecording && !isPaused ? realtimeVoiceMetrics.rhythm_consistency : 0) > 0.7 
                              ? 'bg-gradient-to-r from-green-400 to-green-600' 
                              : (isRecording && !isPaused ? realtimeVoiceMetrics.rhythm_consistency : 0) > 0.4
                              ? 'bg-gradient-to-r from-yellow-400 to-yellow-600'
                              : 'bg-gradient-to-r from-red-400 to-red-600'
                          }`}
                          style={{ 
                            width: `${Math.min(Math.max((isRecording && !isPaused ? (realtimeVoiceMetrics.rhythm_consistency * 100) : 0), 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                      <div className="text-white/60 text-xs">
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Speech Flow</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? (realtimeVoiceMetrics.speech_continuity * 100) : 0).toFixed(1)}%
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${
                            (isRecording && !isPaused ? realtimeVoiceMetrics.speech_continuity : 0) > 0.8 
                              ? 'bg-gradient-to-r from-green-400 to-green-600' 
                              : (isRecording && !isPaused ? realtimeVoiceMetrics.speech_continuity : 0) > 0.6
                              ? 'bg-gradient-to-r from-yellow-400 to-yellow-600'
                              : 'bg-gradient-to-r from-red-400 to-red-600'
                          }`}
                          style={{ 
                            width: `${Math.min(Math.max((isRecording && !isPaused ? (realtimeVoiceMetrics.speech_continuity * 100) : 0), 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                      <div className="text-white/60 text-xs">
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-white text-sm font-medium">Speaking Efficiency</span>
                        <span className="text-[#00ccff] text-sm font-bold">
                          {(isRecording && !isPaused ? (realtimeVoiceMetrics.speaking_efficiency * 100) : 0).toFixed(1)}%
                        </span>
                      </div>
                      <div className="w-full bg-white/20 rounded-full h-3 mb-2">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${
                            (isRecording && !isPaused ? realtimeVoiceMetrics.speaking_efficiency : 0) > 0.8 
                              ? 'bg-gradient-to-r from-green-400 to-green-600' 
                              : (isRecording && !isPaused ? realtimeVoiceMetrics.speaking_efficiency : 0) > 0.6
                              ? 'bg-gradient-to-r from-yellow-400 to-yellow-600'
                              : 'bg-gradient-to-r from-red-400 to-red-600'
                          }`}
                          style={{ 
                            width: `${Math.min(Math.max((isRecording && !isPaused ? (realtimeVoiceMetrics.speaking_efficiency * 100) : 0), 0), 100)}%` 
                          }}
                        ></div>
                      </div>
                      <div className="text-white/60 text-xs">
                ...
                {/* Voice Quality Issues & Suggestions */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Voice Quality Issues */}
                  <div className="p-4 lg:p-6 bg-gradient-to-b from-[#00171f] to-[#003b46] dark:from-[#003b46] dark:to-[#0084a6] rounded-lg">
                    <h3 className="text-white text-lg lg:text-xl font-semibold mb-4">🎯 Voice Quality Issues</h3>
                    
                    {/* Voice Stability Issues */}
                    {(isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0) > 0.02 && (
                      <div className="mb-4 p-3 rounded-lg border-2 bg-red-500/20 border-red-500/50">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-1 rounded-full text-xs font-bold bg-red-500 text-white">
                            High Priority
                          </span>
                          <h4 className="text-white font-semibold">Voice Instability</h4>
                        </div>
                        <p className="text-white/90 text-sm mb-2">
                          Reduce vocal jitter for clearer speech
                        </p>
                        <p className="text-white/70 text-xs bg-white/10 rounded p-2">
                          Current: {(isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0).toFixed(3)} → Target: &lt;0.02
                        </p>
                        <p className="text-red-200 text-xs mt-1">
                          High - affects voice clarity
                        </p>
                      </div>
                    )}
                    ...
                    {(isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0) > 0.1 && (
                      <div className="mb-4 p-3 rounded-lg border-2 bg-yellow-500/20 border-yellow-500/50">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-1 rounded-full text-xs font-bold bg-yellow-500 text-white">
                            Medium Priority
                          </span>
                          <h4 className="text-white font-semibold">Voice Clarity Issues</h4>
                        </div>
                        <p className="text-white/90 text-sm mb-2">
                          Work on reducing vocal shimmer for better articulation
                        </p>
                        <p className="text-white/70 text-xs bg-white/10 rounded p-2">
                          Current: {(isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0).toFixed(3)} → Target: &lt;0.1
                        </p>
                        <p className="text-yellow-200 text-xs mt-1">
                          Medium - affects articulation
                        </p>
                      </div>
                    )}

                    {/* Rhythm Issues */}
                    {(isRecording && !isPaused ? realtimeVoiceMetrics.rhythm_outliers : 0) >= 3 && (
                      <div className="mb-4 p-3 rounded-lg border-2 bg-orange-500/20 border-orange-500/50">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-1 rounded-full text-xs font-bold bg-orange-500 text-white">
                            Medium Priority
                          </span>
                          <h4 className="text-white font-semibold">Rhythm Issues</h4>
                        </div>
                        <p className="text-white/90 text-sm mb-2">
                          Aim for consistent speech segment durations
                        </p>
                        <p className="text-white/70 text-xs bg-white/10 rounded p-2">
                          Current: {isRecording && !isPaused ? realtimeVoiceMetrics.rhythm_outliers : 0} → Target: &lt;3
                        </p>
                        <p className="text-orange-200 text-xs mt-1">
                          Medium - affects speech flow
                        </p>
                      </div>
                    )}
                    ...
                    {(isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0) <= 0.02 && 
                     ...
                     (isRecording && !isPaused ? realtimeVoiceMetrics.rhythm_outliers : 0) < 3 && (
                      <div className="text-center py-4">
                        <p className="text-green-300 text-lg">✅ No Voice Quality Issues Detected!</p>
                        <p className="text-white/70 text-sm mt-2">Your voice quality is within excellent ranges.</p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Practice Exercises */}
                  <div className="p-4 lg:p-6 bg-gradient-to-b from-[#00171f] to-[#003b46] dark:from-[#003b46] dark:to-[#0084a6] rounded-lg">
                    <h3 className="text-white text-lg lg:text-xl font-semibold mb-4">🏋️ Practice Exercises</h3>
                    
                    {/* Voice Quality Practice */}
                    {((isRecording && !isPaused ? realtimeVoiceMetrics.jitter_local : 0) > 0.015 || (isRecording && !isPaused ? realtimeVoiceMetrics.shimmer_local : 0) > 0.08) && (
                      <div className="mb-4 p-3 rounded-lg border-2 bg-blue-500/20 border-blue-500/50">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-1 rounded-full text-xs font-bold bg-blue-500 text-white">
                            Practice
                          </span>
                          <h4 className="text-white font-semibold">Voice Quality Training</h4>
                        </div>
                        <p className="text-white/90 text-sm mb-2">
                          Practice vocal exercises and breathing techniques
                        </p>
                        <p className="text-white/70 text-xs bg-white/10 rounded p-2">
                          Current: Voice quality issues detected → Target: Clear, stable voice
                        </p>
                        <p className="text-blue-200 text-xs mt-1">
                          Improves voice clarity and stability
                        </p>
                      </div>
                    )}

                    {/* Rhythm Practice */}
                    {((isRecording && !isPaused ? realtimeVoiceMetrics.rhythm_outliers : 0) >= 3 || (isRecording && !isPaused ? realtimeVoiceMetrics.speech_continuity : 0) < 0.8) && (
                      <div className="mb-4 p-3 rounded-lg border-2 bg-purple-500/20 border-purple-500/50">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-1 rounded-full text-xs font-bold bg-purple-500 text-white">
                            Practice
                          </span>
                          <h4 className="text-white font-semibold">Rhythm and Flow Training</h4>
                        </div>
                        <p className="text-white/90 text-sm mb-2">
                          Practice speaking in 10-second segments with consistent timing
                        </p>
                        <p className="text-white/70 text-xs bg-white/10 rounded p-2">
                          Current: Irregular rhythm detected → Target: Regular, flowing rhythm
                        </p>
                        <p className="text-purple-200 text-xs mt-1">
                          Enhances speech flow and naturalness
                        </p>
                      </div>
                    )}

                    {/* General Voice Practice */}
                    <div className="mb-4 p-3 rounded-lg border-2 bg-green-500/20 border-green-500/50">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-1 rounded-full text-xs font-bold bg-green-500 text-white">
                          Daily Practice
                        </span>
                        <h4 className="text-white font-semibold">Breathing Exercises</h4>
                      </div>
                      <p className="text-white/90 text-sm mb-2">
                        Practice deep breathing exercises for 5 minutes before speaking
                      </p>
                      <p className="text-white/70 text-xs bg-white/10 rounded p-2">
                        Current: Not done → Target: Better voice control
                      </p>
                      <p className="text-green-200 text-xs mt-1">
                        Reduces vocal jitter and improves stability
                      </p>
                    </div>
                  </div>
                </div>
