        ...
        <h2 className="text-xl font-semibold mb-15">Voice Recorder</h2>
            ...
            <div className="relative flex items-center justify-center">
              ...
              {(isPaused || !isRecording) && (
                <FaMicrophone className={`text-white text-4xl ${(!isRecording || isPaused) ? 'opacity-50' : ''}`} />
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mt-15">
              <button style={{ backgroundColor: "white" }}
                onClick={handlePause}
                className={`p-4 rounded-full shadow-md ${isRecording && !isPaused ? "bg-yellow-500 hover:bg-yellow-600" : "bg-gray-500 cursor-not-allowed"}`}
                disabled={!isRecording || isPaused}
              >
                <FaPause className={`text-black text-2xl ${(!isRecording || isPaused) ? 'opacity-50 cursor-not-allowed' : ''}`} />
              </button>
              <button style={{ backgroundColor: "white" }}
                onClick={handlePlay}
                className={`p-4 rounded-full shadow-md ${isRecording && !isPaused ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-500 cursor-not-allowed"}`}
                disabled={isRecording && !isPaused}
              >
                <FaPlay className={`text-black text-2xl ${isRecording && !isPaused ? 'opacity-50 cursor-not-allowed' : ''}`} />
              </button>
              <button style={{ backgroundColor: "white" }}
                onClick={handleStop}
                className={`p-4 rounded-full shadow-md ${isRecording ? "bg-gray-500 hover:bg-gray-600" : "bg-gray-500 cursor-not-allowed"}`}
                disabled={!isRecording}
              >
                <FaStop className={`text-black text-2xl ${!isRecording ? 'opacity-50 cursor-not-allowed' : ''}`} />
              </button>
            </div>
